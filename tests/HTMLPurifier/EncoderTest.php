...
class HTMLPurifier_EncoderTest extends HTMLPurifier_Harness
{
    ...
    public function assertCleanUTF8($string, $expect = null)
    {
        ...
        $this->assertIdentical(Encoder::cleanUTF8($string), $expect, 'iconv: %s');
        $this->assertIdentical(Encoder::cleanUTF8($string, true), $expect, 'PHP: %s');
    }
    ...
    {
        if (!Encoder::iconvAvailable()) return;
    ...
    {
        if (!Encoder::iconvAvailable()) return;
        if (Encoder::testIconvTruncateBug() !== Encoder::ICONV_TRUNCATES) return;
        $this->config->set('Core.Encoding', 'ISO-8859-1');
        $this->assertIdentical(
    ...
    public function testIconvChunking()
    {
        if (!Encoder::iconvAvailable()) return;
        if (Encoder::testIconvTruncateBug() !== Encoder::ICONV_TRUNCATES) return;
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "a\xF3\xA0\x80\xA0b", 4), 'ab');
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "aa\xE4\xB8\xADb", 4), 'aab');
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "aaa\xCE\xB1b", 4), 'aaab');
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "aaaa\xF3\xA0\x80\xA0b", 4), 'aaaab');
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "aaaa\xE4\xB8\xADb", 4), 'aaaab');
        $this->assertIdentical(Encoder::iconv('utf-8', 'iso-8859-1//IGNORE', "aaaa\xCE\xB1b", 4), 'aaaab');
    }
